...
class MultiScaleInformationBottleneck(nn.Module):
    def __init__(self, input_dim, output_dim, bottleneck_dims=None, dropout_rate=0.2, activation_fn=nn.ReLU):
        super().__init__()
        self.bottleneck_dims = bottleneck_dims if bottleneck_dims else [16, 32, 64]
    ...
    def forward(self, h):
        ...
        for bottleneck in self.bottlenecks:
            ...
            concatenated_features = concatenated_features + scale_features  # 残差连接
        output = self.ffn(concatenated_features)  # 前馈网络
        output = self.norm(output)  # 归一化
        return output


class MultiHeadCoAttentionWithGating(nn.Module):
    def __init__(self, feature_dim, num_heads, dropout_rate=0.2, activation_fn=nn.ReLU):
        super().__init__()
        ...
        self.final_layer = nn.Sequential(
            nn.Linear(num_heads * feature_dim, feature_dim),
            activation_fn(),
            nn.Dropout(dropout_rate)
        )
    ...
    def forward(self, h_protein, h_ligand):
        ...
        ligand_final = self.norm(ligand_final + h_ligand)  # 残差连接 + 归一化
        ligand_final = self.ffn(ligand_final)  # 前馈网络
...
class CombinedModel(nn.Module):
    ...
    def forward(self, h, mask_ligand, batch):
        ...
        for i in range(batch_size):
            ...
            protein_features = h[protein_batch_mask]
            ligand_features = h[ligand_batch_mask]

            protein_features = self.spp(protein_features)
            ligand_features = self.spp(ligand_features)
...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    ...
    def _apply_interaction_pipeline(self, h, mask_ligand, batch, num_interactions=3):
