...
# 定义通用 MLP 结构
def build_mlp(in_dim, out_dim, hidden_dim, num_layers=2, norm=False, act_fn='relu', act_last=False):
    layers = []
    activation = {'relu': nn.ReLU(), 'gelu': nn.GELU()}[act_fn]
    ...
    for i in range(num_layers):
        ...
        if i < num_layers - 1 or act_last:
            if norm and i < num_layers - 1:  # 只在隐藏层加归一化
...
class MultiScaleInformationBottleneck(nn.Module):
    def __init__(self, input_dim, output_dim, bottleneck_dims=[16, 32, 64]):
        super().__init__()
        self.bottlenecks = nn.ModuleList([
            nn.Sequential(
                nn.Linear(input_dim, bottleneck_dim),
                nn.ReLU(),
                nn.Linear(bottleneck_dim, bottleneck_dim),  # 增加一个额外的隐藏层
                nn.ReLU(),
                nn.Linear(bottleneck_dim, output_dim),
            ) for bottleneck_dim in bottleneck_dims
        ])
        self.ffn = build_mlp(output_dim, output_dim, output_dim, num_layers=2)
        self.norm = nn.LayerNorm(output_dim)
        # 可学习的加权系数
        self.weights = nn.Parameter(torch.ones(len(bottleneck_dims)))

    def forward(self, h):
        concatenated_features = h
        for bottleneck in self.bottlenecks:
            concatenated_features += self.weights[i] * bottleneck_output
        return self.ffn(self.norm(concatenated_features))
...
class MultiHeadCoAttentionWithGating(nn.Module):
    def __init__(self, feature_dim, num_heads):
        ...
        self.attn_dim = feature_dim // num_heads

        # 线性投影
        self.protein_proj = nn.Linear(feature_dim, feature_dim, bias=False)
        self.ligand_proj = nn.Linear(feature_dim, feature_dim, bias=False)
        ...
        # 门控机制
        self.protein_gate = nn.Linear(feature_dim, self.attn_dim)
        self.ligand_gate = nn.Linear(feature_dim, self.attn_dim)
    ...
    def forward(self, h_protein, h_ligand):
        # 线性投影
        protein_feat = self.protein_proj(h_protein).view(-1, self.num_heads, self.attn_dim)
        ligand_feat = self.ligand_proj(h_ligand).view(-1, self.num_heads, self.attn_dim)

        # 注意力机制
        ligand_attention = F.scaled_dot_product_attention(ligand_feat, protein_feat, protein_feat)
        ligand_context = torch.einsum('bhd,bhd->bhd', ligand_attention, protein_feat)

        protein_attention = F.scaled_dot_product_attention(protein_feat, ligand_feat, ligand_feat)
        protein_context = torch.einsum('bhd,bhd->bhd', protein_attention, ligand_feat)

        # 门控机制
        ligand_gate = torch.sigmoid(self.ligand_gate(h_ligand)).view(-1, self.num_heads, self.attn_dim)
        protein_gate = torch.sigmoid(self.protein_gate(h_protein)).view(-1, self.num_heads, self.attn_dim)
        ...
        # 更新蛋白质和配体特征
        protein_final = self.protein_update(protein_context.view(-1, feature_dim)) + h_protein  # 直接残差连接
        ligand_final = self.ligand_update(ligand_context.view(-1, feature_dim)) + h_ligand  # 直接残差连接
...
class CombinedModel(nn.Module):
    def __init__(self, input_dim, output_dim, feature_dim, num_heads):
        super().__init__()
        self.spp = MultiScaleInformationBottleneck(input_dim, output_dim)
        self.co_attention = MultiHeadCoAttentionWithGating(feature_dim, num_heads)
    ...
    def forward(self, h, mask_ligand, batch):
        ...
        for i in range(batch_size):
            protein_mask = (batch == i) & (mask_ligand == 0)
            ligand_mask = (batch == i) & (mask_ligand == 1)

            protein_features = self.spp(h[protein_mask])
            ligand_features = self.spp(h[ligand_mask])
        ...
        h_updated = h.clone()
        h_updated[mask_ligand == 0] = torch.cat(protein_final_list, dim=0)
        h_updated[mask_ligand == 1] = torch.cat(ligand_final_list, dim=0)
...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    def __init__(self, num_blocks, num_layers, hidden_dim, n_heads=1, knn=32, num_heads=4,
